-- drop and create tables for the Monash New Smile schema
...
-- creates a table for nurse information
CREATE TABLE nurse (
...
-- adds comments to the nurse table columns
...
-- alter the nurse table to add a primary key constraint
ALTER TABLE nurse ADD CONSTRAINT nurse_pk PRIMARY KEY ( nurse_no );

-- alter the nurse table to add a check constraint called nrsemploystatus_chk to ensure nurse_employstatus is one of 'C', 'F', or 'T'
ALTER TABLE nurse
...
-- inserts a new specialisation into the specialisation table
...
-- inserts a new provider into the provider table
...
-- inserts a new nurse into the nurse table
